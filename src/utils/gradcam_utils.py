...
def grad_cam(input_model,
    ...
    # Process CAM
    cam = cv2.resize(cam, targetSize, cv2.INTER_LINEAR)
...
def compute_saliency(model,
    ...
    # Pre-Process image
    preprocessed_input = load_image(img_path, targetSize=inputSize)
    ...
    gradcam = grad_cam(model, preprocessed_input, cls, layer_name, inputSize)
...
def generate_gradCAM(model, 
    ...
    for i in range((N + batch_size - 1) // batch_size):
        ...
        gradcam[start:end] = grad_cam_batch(model, 
                                            ...
                                            top[start:end],
                                            layer,
                                            (processedimages.shape[1], processedimages.shape[2]))
